 ...
 * @version     1.0.0
 **/
	global $wp_query,$digic_settings;
	...
	$show_rating 	= (isset($_POST['show_rating']) && $_POST['show_rating'] ) ? $_POST['show_rating'] : 0;
	...
	$check_filter = array('only_sale','in_stock','min_price','max_price','paged','orderby','filter_brand','rating_filter');
	if($attribute){	
	...
	if($data){
		...
		foreach($f_data as $key=>$p){
			if($key == 'only_sale'){
			...
			}elseif($key == 'rating_filter'){
				$rating_terms  = array();
				$product_visibility_terms = wc_get_product_visibility_term_ids();
				for ( $i = 1; $i <= 5; $i ++ ) {
					if ( in_array($i, $p) && isset( $product_visibility_terms[ 'rated-'.$i ] ) ) {
				...
				$link = add_query_arg( 'rating_filter', implode( ',', $p ), $link );
				$chosen_attributes['pa_rating']['terms'] = $p;
				$chosen_attributes['pa_rating']['query_type'] = 'IN';
			}
			else{
	...
	ob_start();
		include(WPBINGO_WIDGET_TEMPLATE_PATH.'bwp-ajax-filter/attribute.php');
		$attribute = ob_get_contents();
		$result->result_attribute = $attribute;
	ob_end_clean();	

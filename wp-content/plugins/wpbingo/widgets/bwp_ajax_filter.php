...
class bwp_ajax_filter_widget extends WP_Widget {
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( array_key_exists('show_rating', $new_instance) ){
	...
	function woocommerce_filter_atribute($attribute,$tax_query,$meta_query,$chosen_attributes,$relation,$showcount){
		...
		foreach( $attribute as $att ){
			...
			if ($count_terms > 0) {	
				...
				<div class="content_filter"><ul id="<?php echo esc_attr( 'pa_'.$att ); ?>">
					<?php
								...
								if(isset($tax_attribute->attribute_type) && $tax_attribute->attribute_type == "color"){
									...
									echo '<li class="filter_color ' . ( $option_is_set ?  "active ". esc_attr( $term -> slug ) ."" : "". esc_attr( $term -> slug ) ."" ) . '">';
											echo '	<span style="background-color:'.esc_attr($color).';"> 
														...
														<label class="wpb-tooltip">'. esc_attr( $term -> name ) .'</label>
													</span>';
									echo	'</li> ';
									...
									echo '<li class="filter_image">';
											echo '	<div ' . ( $option_is_set ?  "class='active ". esc_attr( $term -> slug ) ."'" : "class='". esc_attr( $term -> slug ) ."'" ) . '> ';
											echo '	<span '.$bg_image.'> 
														<input value="'. esc_attr( $term -> slug ) .'" name="filter_'.esc_attr( $att ).'" type="checkbox" ' . ( $option_is_set ?  "checked"  : "" ) . '>
													</span>';		
	...
	function woocommerce_filter_brand($id_category,$tax_query,$meta_query,$chosen_attributes,$showcount){
		...
		$category_style = (isset($digic_settings['category_style']) && $digic_settings['category_style']) ? $digic_settings['category_style'] : 'sidebar';
		...
		if($terms){	
			...
			echo '<div class="content_filter"><ul id="pa_brand" class="filter_brand_product">';
			foreach ($terms as $term) {
				$option_is_set     = in_array( $term->slug, $current_values ); 		
				...
				if($count > 0){
					echo '<li>';
							$text_count			= 	$showcount ? '(' . absint( $count ) . ')' : "";		
							echo '<div ' . ( $option_is_set ?  'class="active"' : '' ) . '>';
								echo '<span></span><input value="'.$term->slug.'" name="filter_brand" type="checkbox" ' . ( $option_is_set ?  "checked" : "" ) . '>';
								echo apply_filters( 'woocommerce_brand_nav_count', '<label class="count">'.esc_html( $term->name ).'<mark>'.esc_html($text_count).'</mark></label>', $count, $term );
							echo '</div>';
					echo '</li> ';
				}	
			}
	...
	function woocommerce_rating_filter($id_category,$tax_query,$meta_query,$chosen_attributes,$showcount){
		$rating_html = '';
		$current_values    = isset( $chosen_attributes[ 'pa_rating' ]['terms'] ) ? $chosen_attributes[ 'pa_rating' ]['terms'] : array();
		$rating_html .= '<div class="bwp-filter bwp-filter-rating">';
		$rating_html .= '<h3>'.esc_html__('Ratings','wpbingo').'</h3>';
		$rating_html .= '<div class="content_filter"><ul id="pa_rating">';
		for ( $rating = 5; $rating >= 1; $rating-- ) {
			$option_is_set     = in_array( $rating, $current_values );
			$count = $this->get_count_product_rating( $rating,$tax_query,$meta_query );
			$text_count			= 	$showcount ? '(' . absint( $count ) . ')' : "";
			$rating_html .= '<li data-rating="'.esc_attr($rating).'">';
			$rating_html .= '<div ' . ( $option_is_set ?  'class="active"' : '' ) . '><span></span>';
			$rating_html .= '<input value="'.esc_attr($rating).'" name="rating_filter" type="checkbox" ' . ( $option_is_set ?  "checked" : "" ).'>';
			$rating_html .= '<label class="count star-rating">'.wc_get_star_rating_html( $rating ).'</label>';
			$rating_html .= '<mark>'.esc_html($text_count).'</mark>';
			$rating_html .= '</div></li>';
		}
		$rating_html .= '</ul>';
		$rating_html .= '</div></div>';
	...
	function get_count_product_rating( $rating,$tax_query,$meta_query ) {
		global $wpdb;
		foreach ( $tax_query as $key => $query ) {

...
function digic_custom_css() {
	...
	if (!is_admin()) {
		...
		wp_add_inline_style( 'digic-style-template', implode($custom_css) );
	}
}
add_action('wp_enqueue_scripts', 'digic_custom_css' );
function digic_custom_js() {
	if (!is_admin()) {
		...
		wp_add_inline_script( 'digic-script', $custom_js );
	}
}
add_action('wp_enqueue_scripts', 'digic_custom_js' );